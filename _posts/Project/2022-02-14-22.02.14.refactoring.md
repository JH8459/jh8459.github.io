...
2주의 기간을 예상하였는데 생각보다 빠르게 진행되었다.

이번 한 주 1주일가량 기간이 남아서 클라이언트에서 내가 작성한 코드들 중 리팩토링이 필요한 부분 우선순위를 정리해 보았으며 다음과 같다.
<br>
<br>
...
깊게 학습하고 쓴 코드가 아니기때문에 수정및 보완이 필요할 듯 하다.
...
우선 컴포넌트 생명주기에 관련된 내용이다.<br>
클라이언트쪽 내용은 깊게 공부해 볼 생각까진 없었으나 내가 작성한 코드만큼은 내가 설명할 수 있어야 내 코드라 말할 수 있다고 생각한다.

깊게 공부하여 코드 수정은 하지 못하겠지만 개념정도는 정리 한 뒤 다시 코드를 써보고 싶어서 선정하였다.

그리고 마지막으로 가져갈 Task는 비동기 처리시 로딩중인 표현을 구현 해 볼 예정이다.<br>
이 부분은 어려운 내용은 아니지만 비동기 처리시 로딩 표현은 전반적인 서비스의 퀄리티가 많이 향상 될 듯하여 선정해보았다.

---


우선 오늘은 프로젝트 전반적인 리팩토링 대상 우선순위를 점검하였으며, 필요한 부분들을 먼저 학습부터 해볼 예정이다.
